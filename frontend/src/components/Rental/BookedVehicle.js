...
const BookedVehicle = () => {
  ...
  useEffect(() => {
    axios
      .post("http://localhost:3001/vehicle/fetchVehicleUser", { vendorId })
      .then((response) => {
    ...
    <div className="container-fluid bg-light pt-3">
      ...
      {vehicles.length > 0 ? (
                ...
                <div className="col-md-7 d-flex align-items-center">
                  <div className="card-body p-0">
                    ...
                    <p className="card-text text-success fw-bold m-1">
                      ₹{vehicle.rent}
                      <small className="fw-lighter"> /hr</small>
                    </p>
                    <p className="card-text m-1">
                      <mark className="fw-bold m-0 p-0 py-1">
                    ...
                    {vehicle.rentals.map((rental, i) => (
                      <div key={i} className="mb-3">
                        <p className="card-text m-1">
                          <img
                            src="https://png.pngtree.com/png-clipart/20220429/original/pngtree-pin-location-icon-with-folded-map-png-image_7581594.png" height={40}
                            alt="location icon"
                          />
                          <small className="text-muted">
                            {rental.RentalItems.work_place}
                          </small>
                        </p>
                        <p className="card-text m-1">
                          <strong>Date Of Booking:</strong>{" "}
                          {new Date(rental.RentalItems.date).toLocaleString()}
                        </p>
                        <p className="card-text m-1">
                          <strong>Booked by:</strong> {rental.user.name} -{" "}
                          {rental.user.contactNumber}
                        </p>
                        <p className="card-text m-1">
                          <strong>Work:</strong>{" "}
                          {rental.RentalItems.work}
                        </p>
                      </div>
                    ))}
                  </div>
        ...
        <div className="container py-3">
          <p className="text-center">No booked vehicles found.</p>
        </div>
      )}
    </div>
  );
